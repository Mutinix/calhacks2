...
exports.individualProduct = function(req, res){
    /*
     * CHANGED:
     * We now use Product._id to address an individual product.
     * We don't really need indices or to have our own ids for the products.
     * It's ok if the ids are random alphanumeric strings generated by Mongo.
     */
	var number = req.param('number');
	ProductSchema.findOne({ _id: number}, function(err, product){
		if(err) return console.error(err);
		if(typeof number==='undefined'){
		res.status(404).json({status:'error'});
		}
		else{res.render('individualProduct', {product: product});}
	});
};
/** GET / ADD PRODUCT**/
